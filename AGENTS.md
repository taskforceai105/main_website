...
- The intended primary deployment target is the GitHub Pages user-site repository `ai-det105.github.io`.
- Keep the site deployable as plain static files unless the user explicitly asks for a tooling change.
- Keep root source files Pages-safe for `https://ai-det105.github.io/` while preserving tolerant path handling for local/subpath previews.
...
- GitHub Pages compatibility through the `dist/` static artifact and root-domain hosting.
